...
for (i in 1:250){
  ...
  Yi = sample(which(colSums(G$graph) > 0),1)
  
  # bootstrap draw error terms
  ib = sample(1:nrow(data),nrow(data),replace=TRUE)
  datan = data[ib,]
  
  datan = cbind(datan[,-Yi],datan[,Yi])
  graphn = G$graph[c(setdiff(1:11,Yi),Yi),c(setdiff(1:11,Yi),Yi)]
  weightsn = G$weights[c(setdiff(1:11,Yi),Yi),c(setdiff(1:11,Yi),Yi)]
  ...
  ## learn graph
  DAGn = learn_graph_pc_disc(datan,graphn)
  ...
  save(file="Results_real_CYTO.RData",Gs,do_reg_res,marginal_SV_res,conditional_SV_res,
