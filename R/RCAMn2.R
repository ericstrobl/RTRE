RCAMn2 <- function(data,graph,Yi,nsamp=1000){
  # root cause analysis of marginals
  ...
  for (i in anc ){
    for (j in 1:nsamp){
      ...
      psi1 = apply_RCAM(E,c(W,i),DAG)
      
      if (k == 1){
        psi2 = mean(Y)
      } else{
        psi2 = apply_RCAM(E,W,DAG)
      }
